...
"""This module was written by timtebeek (https://github.com/devjones/PyEtherpadLite/blob/master/src/py_etherpad.py)"""
...
class EtherpadLiteClient:
    ...
    def __init__(self, apiKey=None, baseUrl=None):
        if apiKey is not None and apiKey is not False:
            self.apiKey = apiKey
    ...
    def call(self, function, arguments=None):
        ...
        url = '%s/api/%d/%s' % (self.baseUrl, self.API_VERSION, function)
        ...
        try:
            ...
            print "[EtherpadLiteClient//Noice]:", url, data
            response = opener.open(request, timeout=self.TIMEOUT)
            result = response.read()
            response.close()
        except urllib2.HTTPError as e:
            print "[EtherpadLiteClient//Error]:", e
    ...
    def handleResult(self, result):
        ...
        if 'code' not in result:
            ...
            print "[EtherpadLiteClient//Error]: API response has no code"
        if 'message' not in result:
            #raise Exception("API response has no message")
            print "[EtherpadLiteClient//Error]: API response has no message"
